...
async function fetchMifidIIComplianceFramework() {
  // Search for MiFID II Compliance Framework document
  const query = `*[_type == "servicePage" && (
    title match "*MiFID II Compliance Framework*" ||
    title match "*KI-gestützte Wertpapierdienstleistungs*" ||
    slug.current match "*mifid-ii-compliance-framework*"
  )]{
    _id,
    title,
    "slug": slug.current,
