...
async function verifyMifidProductGovernanceEn() {
  console.log('Verifying MiFID Product Governance EN page...\n')
  
  try {
    const doc = await client.fetch(`*[_id == "mifid-product-governance-en"][0]{
      _id,
      title,
      language,
      __i18n_lang,
    ...
    if (doc.servicesCount !== 6) issues.push(`Services count should be 6, got ${doc.servicesCount}`)
...
verifyMifidProductGovernanceEn()
